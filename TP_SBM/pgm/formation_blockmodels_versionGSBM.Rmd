...
plotMyMatrix(tree_bin,rowLabel = 'Tree', colLabel = 'Tree')
```
...
- We can now extract the parameters of interest, namely (`sbm.tree_bin$connectParam$mu`, $\alpha$, `sbm.tree_bin$blockProp` $\pi$) and the nodes (here tree species) clustering $Z$ (`sbm.tree_bin$memberships`) 
...
plot(sbm.tree_bin,type = 'data',ordered = TRUE,colLabel = 'tree', rowLabel = 'tree' )
``` 
...
plotMyMatrix(tree,rowLabel = 'Tree', colLabel = 'Tree')
```
...
plot(sbm.tree,type = 'data',ordered = TRUE,colLabel = 'tree', rowLabel = 'tree' )
``` 
...
plot(sbm.cov,type='data', ordered = TRUE, colLabel = 'Tree', rowLabel = 'Tree')
``` 

```{r plot BM network tree cov,  echo=TRUE, eval = FALSE}
G <- graph_from_adjacency_matrix(sbm.cov$connectParam$mean, mode = c("undirected"), weighted = TRUE, diag = TRUE)
plot.igraph(G,vertex.size = sbm.cov$blockProp*100,edge.width=sqrt(abs(E(G)$weight)),vertex.color = 1:sbm.cov$nbBlocks, layout = layout_nicely)
``` 
...
## Dataset : fungus-tree interactions
In the fungi-tree interactions network, an edge indicates if a tree species hosts a fungal species. 

- Plot of the incidence matrix

```{r  tree fungis plot, echo=TRUE} 
plotMyMatrix(Mat = fungi_tree,rowLabel = 'fungi', colLabel  = 'tree')
```
...
plot(lbm, type ='data',ordered= TRUE, rowLabel = 'Fungi', colLabel = 'Tree')
``` 
