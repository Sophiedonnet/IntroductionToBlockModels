...
pathtodata <- paste0(getwd(),'/dataThompsonTownsend')
...
linkTosave  = "/home/donnet/WORK_ALL/RECHERCHE/FORMATION_RECHERCHE/FORMATION_RESEAUX/IntroductionToBlockModels/TP_SBM/dataThompsonTownsend"

food_webs <- lapply(
  X = seq_along(my_files),
  FUN = function(i) {
    df <- read.csv(file = paste0(pathtodata,"/",my_files[i]), header = TRUE, row.names = 1)
    write.csv(df,file=paste0(linkTosave ,"/",site_names[i],'.csv'))
    A <- as.matrix(df)
    return(list(
      net = A,
      nr = nrow(A),
